...
then
        ...
        echo "      logs: meaning to generate logs of library interactions"
        echo "            the log files are called log-YYYY-MM-DD.log.xxxx"
        echo "            where YYYY-MM-DD is the current date and"
        echo "            xxxx is a serial numbergenerated by libreoffice."
        exit 1
fi
...
EXECUTE(){
        ...
        then
            ...
            export LD_DEBUG_OUTPUT=./log-$(date +%Y-%m-%d).log
        else
            unset LD_DEBUG
